...
1. 先判断问题落在哪个主题，再只读取对应参考文件，避免一次加载全部资料。
2. 从参考文件里找到最接近的问答、API 或代码片段。
3. 把示例中的单据、字段标识、实体编码、操作标识、插件类型替换成当前项目真实值。
4. 如果文档没有一模一样的案例，组合最接近的模式并明确说明差异和假设。
5. 如果仓库里已经有同类插件或调用方式，优先贴合本地代码风格，不要机械照抄文档示例。
...
- 先不确定落点时：先看 `references/overview.md`

## 检索建议

- 优先按业务对象和动作搜关键词，例如 `单据体`、`子单据体`、`F7`、`选中行`、`刷新`、`操作插件`。
...
- `references/overview.md`: 文档来源、四类主题、检索入口
- `references/pages.md`: 页面相关 390 条段落内容
- `references/interaction.md`: 交互相关 301 条段落内容
- `references/backend-data.md`: 后端数据相关 140 条段落内容
- `references/operations.md`: 操作相关 96 条段落内容
