...
星瀚库根路径：{BASE_PATH}\星瀚帮助中心全量库
苍穹库根路径：{BASE_PATH}\苍穹帮助中心全量库（星瀚文档无机制说明时备用）
...
```
{BASE_PATH}\星瀚帮助中心全量库\search\l1-routing.jsonl
```
...
用 Grep 工具直接搜索以下目录的所有分片文件（约 25 MB，可接受）：

```
{BASE_PATH}\星瀚帮助中心全量库\search\l2-shards\
```
...
```
{BASE_PATH}\星瀚帮助中心全量库\search\l2-shards\{shard_id}.jsonl
```
...
- 星瀚文档有明确说明 → 优先引用星瀚文档
- 星瀚文档无机制说明 → 直接查苍穹库（路径见上方路径说明），无需标注"回落"
- 两者结论一致 → 合并呈现，注明"苍穹/星瀚通用"
...
1. 产品功能说明（来源：专题名 + 文章标题）
2. 操作步骤（若适用）
3. 版本限制（若有）
4. 代码实现（如涉及插件编写，先用元数据索引确认标识；若需要 BOS 接口/事件机制，再按需参考苍穹官方开发文档，必要时结合 kingdee-cangqiong-dev-tips 组织代码）
5. 官方链接：{official_url}
...
| 文件 | 原因 |
|------|------|
| `search\l2-articles.jsonl`（大文件） | 体积过大，通过 l2-shards 分片访问 |
